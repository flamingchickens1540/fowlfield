...
{#if team_num != 0}
    <div class="team-card" style="--colorA:{alliance == 'red' ? 'rgb(218, 56, 50)' : 'rgb(42, 100, 173)'}; --colorB:{alliance == 'red' ? 'hsl(2, 63%, 45%)' : 'hsl(213, 64%, 35%)'}">
        ...
        <div class="conts">
            <div class="team-info-label">Team Name</div>
            <div class="team-info-item">{$teamList[team_num]?.team_name?.get()}</div>
            <div class="team-info-label">Robot Name</div>
            <div class="team-info-item">{$teamList[team_num]?.robot_name?.get()}</div>
            {#if $type == 'qualification'}
                <div class="team-info-label">Team Rank</div>
                <div class="team-info-item">{getOrdinal($rankingsmap[team_num])}</div>
            {:else}{/if}
        </div>
    </div>
{/if}
